...
func TestNewPowerStoreAdapter(t *testing.T) {
	t.Run("ValidClient", func(t *testing.T) {
		client := fake.NewClientBuilder().Build()
		translator := translation.NewEngine()

		adapter, err := NewPowerStoreAdapter(client, translator)
	...
	t.Run("NilClient", func(t *testing.T) {
		_, err := NewPowerStoreAdapter(nil, nil)
		assert.Error(t, err)
	...
	t.Run("NilTranslator", func(t *testing.T) {
		...
		adapter, err := NewPowerStoreAdapter(client, nil)
		assert.NoError(t, err, "Should create translator if nil")
		assert.NotNil(t, adapter)
	})
}

func TestPowerStoreAdapter_CreateReplication(t *testing.T) {
...
func TestPowerStoreAdapter_Operations(t *testing.T) {
	scheme := runtime.NewScheme()
	client := fake.NewClientBuilder().WithScheme(scheme).Build()
	translator := translation.NewEngine()
	...
	ctx := context.Background()
	uvr := createTestUVRForPowerStore("test-ops", "default")
	...
	t.Run("ResyncReplication", func(t *testing.T) {
		...
		// May fail without CRD
		_ = err
	})

	t.Run("PauseReplication", func(t *testing.T) {
		err := adapter.PauseReplication(ctx, uvr)
		_ = err
	})

	t.Run("ResumeReplication", func(t *testing.T) {
		err := adapter.ResumeReplication(ctx, uvr)
		_ = err
	})
}

func TestPowerStoreAdapter_StateTranslation(t *testing.T) {
	client := fake.NewClientBuilder().Build()
	translator := translation.NewEngine()
...
func TestPowerStoreAdapter_MetroReplication(t *testing.T) {
	client := fake.NewClientBuilder().Build()
	translator := translation.NewEngine()
	...
	// Create UVR with synchronous mode (Metro)
...
// Helper function
func createTestUVRForPowerStore(name, namespace string) *replicationv1alpha1.UnifiedVolumeReplication {
